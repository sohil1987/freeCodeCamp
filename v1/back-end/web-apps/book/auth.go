...
func doLoginOrCreate(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, baseURL+"book/logged/?user="+user, 301)
}

func createUser(db *sql.DB, user, pwd string) error {
	salt := make([]byte, 16)
	rand.Reader.Read(salt)
	_, err := db.Exec("INSERT INTO book.users (Username, PassSalt, PassMd5)     values (?, ?, ?)", user, salt, saltHash(salt, pwd))
	if err != nil {
...
func authenticateUser(db *sql.DB, user, pwd string) error {
	var salt, hash []byte
	row := db.QueryRow("SELECT PassSalt, PassMd5 from book.users WHERE Username=?", user)
...
func dbGetAllUsers(db *sql.DB) {
	rows, _ := db.Query("SELECT Username, PassSalt, PassMd5 FROM book.users")
	defer rows.Close()
	var user string
	var salt, hash []byte
	for rows.Next() {
